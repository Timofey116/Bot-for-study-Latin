...
# Получаем токен для бота из отдельного файла
...
def buttons(message):#Обработчик текстовых сообщений. Определяет, какую кнопку нажал пользователь, и выполняет соответствующее действие.
    ...
    elif message.text == "📜Data verbum":# Отправляет случайную флеш-карту пользователю.
    ...
    elif message.text == "🐺Сitatus": #Отправляет изображение со случайным афоризмом
...
#Отправляет флеш-карту пользователю и сохраняет его состояние.
#Если пользователь еще не получал флеш-карту, создается новая запись.
#Флеш-карта отправляется с инлайн-клавиатурой для взаимодействия.
#Args:
...
#Получает часть флеш-карты в зависимости от состояния (лицевая или обратная сторона).
#Разделяет флеш-карту на части и возвращает соответствующую часть в зависимости от состояния.
#Args:
        #index: Индекс комплимента в словаре.
        #state: Состояние карточки ('front' или 'back').
#Returns:
        #str: Лицевая или обратная часть флеш-карты.


def create_inline_markup(): #Создает инлайн-клавиатуру с кнопкой для переворота флеш-карточки.
    #Returns:
